...
### Add required variant meta fields:
    ...
    Up the top right, click "Add definition"
    Call it "Variant Markup Percentage"
    Set the "Type" to "Decimal"
    Add a description such as "The markup added to the total sheet stock price as a percentage"
    Click "Save" up the top of the page.
    
    Go back to the previous page ("Variant metafield definitions", not "Metafields and metaobjects")
    
    Add another variant metafield, call it "Cut Surcharge"
    Set the "Type" to "Money"
    Add a description such as "The per line item cut surcharge for sheet/plate metal stock"
    Click "Save" up the top of the page.
    
    Go back to the "Metafields and metaobjects" page (not the "Variant metafield definitions" page)
    Click on "Products" metafields
    Add the following 6 Definitions:
...
# 2. Add a new product:
	...
	Update each of the variants, click on each in turn and add the following:
		Ensure there is enough inventory for testing, or leave it empty until Jesse is ready to test.
...
# 3. Add a template including the app ui
    ...
    Open the "Home page" drop down, mouse over and click "Products", you will need to create a new template called "sheetstock".
    While creating the "sheetstock" template, on the left, under "Template", expand the "Product information" (if necessary) and click "Add block"
...
# 4. Verify everything has worked.
    ...
    Save the URL and share it with me so I can view the page also.
